...
logging.info(
    f"=== Starting LLM Experiment ({datetime.now().isoformat()}) ==="
)
...
if __name__ == "__main__":
    ...
    #     "customized_prompt": "data/custom_prompt_1.txt"
    # }
    ...
    with wandb.init() as run:
        ...
        summary_results = {
            "mean": sim_results["LLM_name_GO_term_sim"].mean(),
            "median": sim_results["LLM_name_GO_term_sim"].median(),
            "std": sim_results["LLM_name_GO_term_sim"].std(),
            "min": sim_results["LLM_name_GO_term_sim"].min(),
            "max": sim_results["LLM_name_GO_term_sim"].max(),
            "count": sim_results["LLM_name_GO_term_sim"].count()
        }

        wandb.run.summary["mean_LLM_name_GO_term_sim"] = summary_results['mean']
        wandb.run.summary["median_LLM_name_GO_term_sim"] = summary_results['median']
        wandb.run.summary["std_LLM_name_GO_term_sim"] = summary_results['std']
        wandb.run.summary["min_LLM_name_GO_term_sim"] = summary_results['min']
        wandb.run.summary["max_LLM_name_GO_term_sim"] = summary_results['max']
        wandb.run.summary["count_LLM_name_GO_term_sim"] = summary_results['count']
